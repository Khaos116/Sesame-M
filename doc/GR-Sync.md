...
- **版本伪装 Hook**（GR2026: [hook/ext/VersionHook.java](../../Gr/Sesame-GR2026/app/src/main/java/io/github/lazyimmortal/sesame/hook/ext/VersionHook.java)）：通过 hook `PackageManager.getPackageInfo()` 伪造支付宝版本号。起初按 Sure-Xu 文档的判断（属于特定版本/个人偏好，不直接覆盖）不迁入；后来 my_dev 移植过一版（默认开启→改回默认关闭），2026-09-21 真机验证无效（伪装后服务端仍下发需对准图片的滑块）后**整体删除**，不再迁入，见 CHANGELOG.md。
