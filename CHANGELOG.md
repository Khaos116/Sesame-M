...
- fix：拼图验证码截图目录改为 `sesame-M/puzzle/<账号ID>/`（`FileUtil.getCurrentUserPuzzleDirectory`），不再放在 `log/<账号ID>/puzzle/` 下：用户在旧位置找不到文件，且放在 log 下清理日志时会被一起删。同时把之前 CHANGELOG 里写的截图位置更正为新目录。
