...
- chore：版本号 1.1.6 → 1.1.7（`gradle.properties`），tag `v1.1.7`。1.1.6 → 1.1.7 之间是 v1.1.6 之后的 51 个提交（含 2 次合并 `origin/MIUIX-api102`），主要是拼图验证码增强（重试/次数可配置/H5 触发/截图管理）、账号名命名的日志目录与导出文件、上游配置保存与界面更新。tag `v1.1.6` 仍指向当时的版本号提交，之后的改动全部归在 1.1.7。
