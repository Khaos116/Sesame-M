...
- ui：主页四个 Tab 顶部右侧的“当前账号: ”前缀去掉，只显示账号名称与账号（`TabTitleRow` 是四个 Tab 共用的，一处改动全覆盖；取不到账号时仍显示“未知账号”）；首页顶部标题 `Sesame-M` 改为 `芝麻粒M`（按用户要求的写法；`AGENTS.md` 里写的品牌名是“芝麻粒-M”带连字符，如需统一再改）。日志页/配置页/设置页的标题（日志、配置、设置）不变。
