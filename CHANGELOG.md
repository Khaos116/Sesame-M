...
- fix（未提交）：四审 `b76684c8` 那次合并，我自己合入的 `defaultCount` 逻辑有个更严重的问题——比上游原来的 bug 还大。`defaultCount = if (withCount) (liveField.valueRangeMin.toInt() ?: 1) else 1` 想的是"取字段自身下限当默认值"，但 `SelectAndCountModelField` 的默认构造函数（`SelectAndCountModelField.java:37/41`，五参数那两个重载）硬编码 `min=0, max=100`，只有显式传了 `(min,max)` 的字段（合种浇水两个、`waterFriendList`/`wateredFriendList` 传的是 `1,3`）才不是 0。项目里绝大多数 `SelectAndCountModelField` 字段——`feedFriendAnimalList`（帮喂小鸡）、`visitFriendList`（送麦子）、`gameCenterBuyMallItemList`、`orchardSpreadManureSceneList`、`protectTreeList`、`reserveList`、`protectOceanList`、`protectMarathonList`、`protectNewAncientTreeList`、`vitality_ExchangeBenefitList`、`rpcRequestList`——全部走的是没传 min/max 的默认构造函数，`valueRangeMin` 实际是 0。结果：用户在配置页新勾选这些字段的任意好友/场景后，只要没有手动拖一下滑块，保存下来的次数就是 0；业务侧（如 `AntFarm.java:2226` `Status.canFeedFriendToday`：`count < countLimit`）把 0 次当"今日已达上限"直接跳过，等于勾了等于没勾，用户完全看不出来——静默失效，比上游原来"次数控件消失"那个 bug 更隐蔽也更严重。改法：只有 `useInputBox`（合种浇水那两个字段，本来就允许/需要 0 作为合法默认值）才取 `valueRangeMin`，其余所有字段维持原来写死的默认值 `1`。顺带补了两处配套问题：①`checks/check_merge_config.py` 的测试桩类之前把 `valueRangeMin` 默认写成 `1f`，跟生产环境默认构造函数实际是 `0f`不符，等于测试永远走不到这个 0 的分支，掩盖了这个 bug；改成 `0f`（对齐生产默认值），并加断言"非 useInputBox 字段默认值必须是 1"、专门测一遍 `cooperateWaterList` 默认值应为 0。②数值输入框（合种浇水专用，`MiuixSelectionEditActivity.kt` 的 `useInputBox` 分支）之前没设键盘类型（会弹全键盘不是数字键盘）也没按 `valueRangeMax` 做上限截断，补了 `KeyboardOptions(keyboardType = KeyboardType.Number)` 和按 `valueRangeMax` 的输入截断。回归全过（含改过的 `check_merge_config.py`），Debug 编译（Java+Kotlin）通过，未做真机验证。
- merge `b76684c8`：再次合并 `origin/MIUIX-api102`（ee219f3a → d4419981，上游 1 个新提交 `0c7e0484`"合种浇水列表支持逐项设置浇水量"，作者自述"个人未参加合作，未实测"）。1 处冲突 + 1 处发现的真 bug：`MiuixSelectionEditActivity.kt` 的 `withCount` 判断，上游改法是 `liveField.type == "SELECT_AND_COUNT" && (code 在 4 个合种/浇水相关字段名单里)`——但 `withCount` 这个变量同时控制着全项目所有 `SelectAndCountModelField` 字段（`rpcRequestList`/`feedFriendAnimalList`/`gameCenterBuyMallItemList`/`visitFriendList`/`protectMarathonList`/`protectNewAncientTreeList`/`protectTreeList`/`reserveList`/`protectOceanList`/`orchardSpreadManureSceneList`/`vitality_ExchangeBenefitList` 等十几个跟合种毫无关系的"选填+次数"字段）的次数编辑 UI 是否显示；照抄会导致这些字段的次数输入控件全部消失，是一次没测出来的大范围回归。合并时把 `withCount` 判断改回原来的"按类型"（`SELECT_AND_COUNT`/`SELECT_AND_COUNT_ONE`），只保留上游真正新增的 `useInputBox`（仅 `cooperateWaterList`/`cooperateWaterTotalLimitList` 两个字段用数值输入框代替滑块，因为这两个字段取值范围到 1000，滑块不好操作）。另外初始勾选项计数那个 `when` 分支合并时也顺手避免了一个我自己合并中途差点引入的问题：那个分支的判断不能改成 `withCount`（会跟下面 `SELECT_AND_COUNT_ONE` 专用分支抢着执行，数据形状不一样——一个是 `Map`，一个是 `KVNode`），保持按类型判断。`checks/check_merge_config.py` 里复现真实逻辑的 `SelectionCheck.kt` 内嵌夹具没跟着上游改动更新（上游提交没碰 checks/），编译报 `defaultCount` 未解析；照着 `@@WITH_COUNT@@` 已有的动态提取模式加了 `@@DEFAULT_COUNT@@`，从真实源文件正则抠那一行，夹具的 `SelectAndCountModelField` 桩类也补上 `valueRangeMin` 字段。GMT+8/JSON 创建/JSON 读取三项检查：本次合并新增代码无违规；`GeminiAI` 未受影响。回归全过（含修好的 `check_merge_config.py`），Debug 编译通过，未做真机验证。
- fix（未提交）：对 `5bf5d90b` 合并（含上游 `38de303d`/`d1ad4438`/`35a4a4da`/`f8fa8e9f`/`ca1621c7`）做审查，修复 4 处问题。
  1. **P0 崩溃**：`ApplicationHook.java:470`/`893` 两处仍是 `if (initHandler(true)) {...}`。`f8fa8e9f` 把 `initHandler` 改成主线程调用即转后台线程、立刻 `return null`（`Boolean` 包装类型），这两处都在主线程（`Service.onCreate` 钩子、`mainHandler.postDelayed`）调用，`if(Boolean)` 拆箱 `null` 必抛 `NullPointerException` 崩支付宝进程。同类调用点 `268` 行早改成不判断返回值直接调用（注释写明"重载在后台线程执行...此处无需依赖返回值"），470/893 是合并时漏改的。已按同一模式改掉：直接调用不判断返回值，470 处 `init=true` 本就是 `initializeHandler` 成功时自己会置的冗余赋值，一并删掉；893 处"用户已切换"日志/Toast 改成触发即报（不代表加载结果，加载成功/失败 `initializeHandler` 自己会 Toast）。
  2. **P2 数据丢失**：`AppConfig.java` 字段 `closeCaptchaDialogVPN` 在 `d1ad4438` 改名为 `closeCaptchaDialog`，`AppConfig.load()` 用 Jackson `readerForUpdating` 增量更新，老配置文件里的旧字段名对不上新字段，老用户之前若手动关过这个开关会被静默重置为默认 `true`。加 `@JsonAlias("closeCaptchaDialogVPN")` 接住旧字段名。
  3. **P2 显示缺陷**：`MiuixMainActivity.kt` 的 `accountDisplayName` 用 `it.showName ?: it.account`，但 `UserEntity.showName` 在昵称/备注/真实姓名都缺时兜底到 `realName`，`realName` 可能是空字符串 `""`（非 null）——`?:` 对空字符串不生效，会显示成 `(account)` 这种带头部空白的怪名字。改成 `if (it.showName.isNullOrEmpty()) it.account else it.showName`。
  4. **文档**：`CHANGELOG.md` 里 `5bf5d90b` 那条合并记录当时写的是"（待提交）"占位符，合并其实已经提交，回填成真实 commit hash。
  另外复核了审查里提到的两点，判断不需要改动：`ApplicationHook.java` 里 `scheduleAccountReload`/`Activity onResume` 外层 `try(TaskLifecycle.Work)` 在主线程转发后台线程后确实不再覆盖整个重载过程，但 `runInit` 内部会用 `enterInitialization(owner)` 重新做一次实时准入检查，账号切换期间该检查会失败并安全跳过（不会用错账号状态执行），不存在踩坏数据的路径，只是小概率丢一次触发、下次 `onResume`/轮询会自然重试；`new Thread(() -> runInit(...))` 缺 try/catch 顺手补了一层（捕获后复位 `initializing`、记日志），`runInit`/`initializeHandler` 内部本来就有 `catch(Throwable)`，这层只是双保险。
  回归：`account_lifecycle`、`audit_regressions`、`check_merge_config`、`check_account_switch`、`check_rpc_guard` 全过，Debug 编译（Java+Kotlin）过，未做真机验证。
- fix（未提交）：对上一条的修复做二次复查，发现并修掉 3 处遗留问题。①`MiuixMainActivity.kt` 的 `accountDisplayName` 上次只把 `showName` 判空兜底到 `it.account`，但外层字符串模板 `"${...}(${it.account})"` 没跟着改，昵称缺失时拼成 `账号(账号)` 重复显示；改成昵称非空时才拼 `昵称(账号)`，昵称空时只显示账号本身。②`ApplicationHook.java:458` `init = initHandler(true, owner)` 把 `Boolean` 直接赋给基本类型 `boolean`，这条调用路径目前始终在 `AccountSwitchController` 的后台线程触发、不会拿到 null，但写法本身留了拆箱隐患；改成 `Boolean.TRUE.equals(...)` 显式判空。③`scheduleAccountReload` 里 `initHandler(true)` 前置校验（未登录/无闹钟权限/正在初始化中）没过时会同步返回 `false`，此时并没有真的触发重载，但下面的"用户已切换" `Log.record`/`Toast` 之前是无条件执行的，会出现"用户未登录"和"用户已切换"背靠背弹的矛盾提示；改成只在返回值不是 `false`（触发了后台重载，或结果稍后自报）时才提示。回归全过，Debug 编译过，未做真机验证。
- fix（未提交）：三审。①`accountDisplayName` 上一版只处理了"昵称缺失"，没处理"`account` 也可能是 null/空串"——`UserEntity.account` 来自反射读取，实际会缺失；`showName` 非空但 `account` 为 null 时字符串模板会把 null 拼成字面 `"张三(null)"`，`showName`/`account` 都是空串时 `if` 分支直接返回 `""`（非 null）导致标题空白，绕过了外层 `?: folderOrUid` 兜底。改成 `name`/`acc` 各自判空取值后按四种组合分别处理，都缺失时兜底 `folderOrUid`。②清掉仓库根目录一个 0 字节的未跟踪文件 `0`（本次会话某条 shell 命令重定向误产生的，不是业务文件，直接删除，不影响任何代码）。回归全过，Debug 编译（Kotlin）无新增警告。
- fix（未提交）：`AntOrchard.java` 的静态黑名单 `ORCHARD_TASK_BLACKLIST`（14 项已知失效/需要前端行为配合的农场任务，含今天异常反馈里报错的 `70000`"逛好物最高得1500肥料"）从未被读取——`handleTaskList` 里真正生效的跳过判断只查了动态黑名单 `AntOrchardTaskList.getValue().contains(title)`，静态集合定义了但没接进这条判断。依据：账号2 `error.2026-09-22.176_78.log` 里 `com.alipay.antiep.finishTask`（sceneCode=BABA_FARM_TASK, taskType=70000, source=ANTFARM_ORCHARD）今天 08:50/09:48/10:00 连续 3 次收到 `code:400000001 desc:任务全局配置不存在`，跟静态黑名单的注释精确对上。改法：`handleTaskList` 的跳过条件加上 `ORCHARD_TASK_BLACKLIST.contains(title/groupId/taskId)`（静态集合里混了任务 ID 和标题，三个字段都查一遍）。跳过时不发 RPC，不算失败，不影响动态黑名单的"连续命中确认"机制（两套并行，互不冲突）。回归：`account_lifecycle`、`audit_regressions`、`check_merge_config` 全过，Debug 编译过。
- 同一批异常反馈里另外两类**证据不足，未处理**：①`com.alipay.antieptask.finishTaskantorchard`/`receiveTaskAwardantorchard`（金豆模块，sceneCode=GOLDEN_BEAN_MASTER_TASK，taskType=GOLDENBEAN_GAME_ZH0_LYJX_V1/ZH0_NCSCC/GOLDEN_BEAN_TASK_WAKUANG）：`rpc-failures.2026-09-21...json` 里各只出现 1 次，`message` 是聚合器自己合成的"响应未提供错误原因"（原始响应大概率字段为空），`MessageUtil` 里 `GoldenBeansTaskList` 分支靠 `code`/`desc` 判定，字段全空时既不会拉黑也不会计入"待确认"，现状是这条错误对现有机制完全不可见，单次出现也不够判断是不是临时的；②农场"子任务"里 `sceneCode=10021 taskType=104321`（`AntOrchard.java:1341` 附近 `switch(actionType)` 的 `VISIT` 分支，走 `childTaskId` 直接调 `finishTask`，这条路径本来就没有任何黑名单跳过逻辑，静态/动态黑名单都碰不到)也是 `400000001 任务全局配置不存在`，3 次都在今天，没有跨天数据，不确定是activity当天没配置还是真死了。这两类需要再攒几天同样报错的数据才能加规则，不直接改。
- 其余异常反馈条目（`resultCode 102` 服务器繁忙类、`error 48` 网络不可用、`donation resultCode 218` 没有指定标的物、能量罩保护、行走限速提示、`V07` 权益已领取）都是已有退避/静默跳过逻辑覆盖的正常瞬时状态，逐条核对代码后确认不需要改动。
- merge `5bf5d90b`：再次合并 `origin/MIUIX-api102`（3066428a → ee219f3a，上游含 `ca1621c7` 修复顶部/配置页昵称显示为 `null`）。根因：`f133ba53` 带进的 `UserIdMap.buildSelfFromAccountModel()` 反射只能稳定拿到当前支付宝的 `userId`/`account`，昵称/姓名字段拿不到，好友列表兜底又只判 `selfEntity == null`（半成品 entity 已非 null），正确昵称被挡在外面；`UserEntity.showName` 昵称/备注都缺时又直接是 null，未做兜底。上游 `ca1621c7` 改法：好友循环遇到自己改用 `mergeSelfEntity` 合并（uid/account 用账号模型、昵称/姓名优先取好友库），`UserEntity` 昵称备注都缺时兜底用真实姓名，`maskName`/`fullName` 空安全拼接。3 处冲突：①`UserIdMap.java` 自动合并成功、无冲突；②`ApplicationHook.java`——`f8fa8e9f`（上游）把 `initHandler` 改成主线程转后台线程执行避免切号卡死界面，和本分支已有的 `TaskLifecycle` 并发准入包装（`initHandler(force, owner)`/`enterInitialization`）撞了同一处重命名，手工合并为“主线程判断转后台线程 + 后台线程内仍走 TaskLifecycle 守卫”，双方诉求都保留，`initializing` 复位挪到 `runInit` 的 `finally`；③`MiuixMainActivity.kt` 配置页账号列表——沿用上次 `f133ba53` 的既有决定，保留本分支 `accountDisplayName` + “UID:”副标题结构（下游 `items.forEach` 按 `Pair` 解构，上游改的 `Triple` 类型不兼容），放弃上游这处的结构改动；额外把 `accountDisplayName` 的字符串拼接也补上 `showName ?: account` 空安全，堵住残余的字面 "null"。GMT+8/JSON 创建/JSON 读取三项检查：合并涉及的 10 个文件里新增代码无 `Calendar`/`new JSONObject`/裸 `get*()`；`GeminiAI` 未受影响。Debug 编译（Java+Kotlin）通过，9 项回归脚本全部 PASS，未做真机验证。
