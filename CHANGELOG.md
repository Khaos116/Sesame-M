...
- 未提交：新增按账号/GMT+8 日期聚合的 RPC 异常统计 JSON，异常日志页支持“导出统计”；记录任务 ID、错误及次数供人工完善黑名单，不改变现有请求策略。
- 未提交：全部日志分类的单条卡片支持长按自由选择复制；核对截图确认庄园抽奖领奖 `102` 系统繁忙未命中现有退避，暂未修改 RPC 策略。
- docs：AI 必读规则新增每次合并必查 GMT+8、MyUtils JSON 创建及 `.opt*()` 安全读取，要求覆盖自动合并文件并记录处理结果与例外。
